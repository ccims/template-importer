...
export class IssueTemplateHandler extends TemplateHandler<IssueTemplateDefinition, FullIssueTemplateInfoFragment> {
    async createTemplate(definition: IssueTemplateDefinition): Promise<FullIssueTemplateInfoFragment> {
        ...
        const assignmentTypes =
            definition.assignmentTypes ??
            defaultValue?.assignmentTypes?.nodes.map((node) => ({
                name: node.name,
                description: node.description
            })) ??
            [];
        const issuePriorities =
            definition.issuePriorities ??
            ...
            [];
        const issueStates =
            definition.issueStates ??
            defaultValue?.issueStates?.nodes.map((node) => ({
                name: node.name,
                description: node.description,
                isOpen: node.isOpen
            })) ??
        ...
        const issueTypes =
            ...
            defaultValue?.issueTypes?.nodes.map((node) => ({
                name: node.name,
                description: node.description,
